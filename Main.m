...
%% create optical parameters used for generator
% move data to GPU
k = (2*pi/IS.lambda);
...
if gpu_flag == 1
    ...
    z_emit = gpuArray(IS.z_emit);
    opt_phase_mat = gpuArray(opt_phase_mat);
    q_cord = gpuArray(q_cord);
...
tmp_stack = [];
for j = 1:size(IMG_T,3)
    tmp = padarray(IMG_T(:,:,j), floor([size(IS.circmask,1) - (size(IMG_T,1)), size(IS.circmask,2) - (size(IMG_T,2))]/2),'both');
    if mod(size(IS.circmask,1),2)==0
