...
while iter<=maxiter
    ...
    elseif sample_flag ==3 % at worst corr2
        ...
        for z_ind = 1:length(grid_sampling)
            ...
            %
            tmp_data = (data(:,:,grid_sampling(z_ind))).*(data(:,:,grid_sampling(z_ind))~=0);
            % thr mask
    ...
    elseif sample_flag ==4 % adaptive sampling
        ...
        CDF = [0,cumsum(p_accu_grad)];
        %generate uni random point
        R_uni = rand(point_num,1);
        %
        out_val = interp1(CDF,[0:1/(length(CDF)-1):1],R_uni'); %spans zero to one
    ...
    if  mod(iter,30) == 1 && plot_flag
        ...
        for z_ind = 1:length(ind_opt)
            ...
            subplot(length(ind_opt),2,countS);
            countS=countS+1;
            imagesc([data(round(end/2-IS.plotsize/2):round(end/2+IS.plotsize/2),round(end/2-IS.plotsize/2):round(end/2+IS.plotsize/2),(ind_opt(z_ind)))]);
    ...
    if (iter == floor(IS.SGDiter/2)) && IS.update_Signal==1
            Nph_opt_flag = 1;
    end
    %% estimate gblur
