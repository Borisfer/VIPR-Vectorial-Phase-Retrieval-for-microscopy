...
% funval = CostML(data,psfmdl,theta)
...
%%  mle
%     funval = -sum(data(:).*log(im_model(:)) - im_model(:));
%% sum of gaussians MLE
%     funval = sum(log(im_model(:)+std_Gbg.^2)+0.5*((data(:)-im_model(:)).^2)./(im_model(:)+std_Gbg.^2));
%% L2
funval = mean(Diff(:));
end
