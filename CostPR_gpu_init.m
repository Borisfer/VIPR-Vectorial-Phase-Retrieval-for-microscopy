        ...
        if sum(IS.p_vec) == 0 % freely rotating - superposition solution
            ...
            for div_pol = 1:floor(size(IS.g_bfp,3)/3)
                ...
                for g_id = (1:3)+3*(div_pol-1)
                    ...
                    g_img(:,:,g_id) = 1/N.*(fft2((g_bfp_init(:,:,g_id).*normfact)).*IS.p_vec(g_id-3*(div_pol-1)));
                end
            end
        end
    ...
    if vec_model_flag 
        if sum(IS.p_vec) == 0 % freely rotating - superposition solution
            ...
            for div_pol = 1:floor(size(IS.g_bfp,3)/3)
                ...
                for g_id = (1:3)+3*(div_pol-1)
                    grad_tmp(:,:,g_id) = 2*1/N*real(fft2(ifftshift(data_z0).*1i.*Conj_fact).*g_bfp_init(:,:,g_id)); %%
                end
            end
        end
        grad(:,:,z_ind) = sum((grad_tmp),3);
    else  % scalar solution
        grad(:,:,z_ind) = 2*1/N*real(fft2(ifftshift(data_z0).*1i.*conj(Eimg)).*Ebfp);
    end
