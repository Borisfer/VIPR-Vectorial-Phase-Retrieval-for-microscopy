...
if vec_model_flag == 1
    ...
    if sum(IS.p_vec == 0) == 3 % freely rotating - superposition solution
        ...
        for g_id = 1:size(IS.g_bfp,3)
            P_fact(g_id) = sum(sum(abs(circ.*g_bfp(:,:,g_id).*BFP_Phase).^2));
        end
        normfact = sqrt((IS.Signal))./sqrt(sum(P_fact));
        
        for g_id = 1:size(IS.g_bfp,3)
            g_bfp(:,:,g_id) = g_bfp(:,:,g_id).*BFP_Phase.*normfact.*circ;
        ...
        for div_pol = 1:floor(size(IS.g_bfp,3)/3)
            for g_id = (1:3)+3*(div_pol-1)
                P_mat(:,:,g_id) = g_bfp(:,:,g_id).*BFP_Phase.*IS.p_vec(g_id-3*(div_pol-1));
            end
    ...
    % normalization
    Pupil = Pupil* sqrt((IS.Signal))/ sqrt(sum(abs(Pupil(:)).^2));
