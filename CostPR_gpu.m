function [out,grad,Nph_max_fact] = CostPR_gpu(phase_mask,IS,q,data,std_stack,Nph_vec,cost_function_flag...
    ,gBlur ,k,opt_phase_mat,g_bfp_init,vec_model_flag,int_cos,circ,circ_sc,Nph_opt_flag)
...
%     data_z0 = data_z0+Noise_mat;
    ...
    if vec_model_flag
        ...
        % normalization
        Ebfp = Ebfp* sqrt(Nph/ (sum(abs(Ebfp(:)).^2)));
...
%         nom = ((tmp+std_Gbg.^2)-2*(tmp+std_Gbg.^2).*(data_z0-tmp)-(data_z0-tmp).^2).*(data_z0~=0);
    ...
    dcost_dIimg = imfilter(dcost_dtmp,gBlur,'replicate');
    ...
    if vec_model_flag
        if sum(IS.p_vec) == 0 % freely rotating - superposition solution
            ...
            for div_pol = 1:floor(size(IS.g_bfp,3)/3)
                ...
                for g_id = (1:3)+3*(div_pol-1)
                    grad_tmp(:,:,g_id) = 2*1/N*real(fft2(ifftshift(dcost_dIimg).*1i.*Conj_fact).*g_bfp(:,:,g_id).*IS.p_vec(g_id-3*(div_pol-1))).*Nph_max_fact(z_ind); %%
                end
            end
        end
        grad(:,:,z_ind) = sum((grad_tmp),3);
    else % scalar model 
        grad(:,:,z_ind) = 2*1/N*real(fft2(ifftshift(dcost_dIimg).*1i.*conj(Eimg)).*Ebfp).*Nph_max_fact(z_ind);
    end
